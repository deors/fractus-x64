...
int main(int argc, char **argv)
{
    ...
    if (argc < 2 || strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "--help") == 0) {
        print_usage((argc > 0 && argv[0] != NULL) ? argv[0] : "drsg2bmp");
        return (argc < 2) ? 1 : 0;
    }
    ...
    if (argc >= 3) {
        ...
        replace_or_append_extension(input_path, ".bmp", output_path, sizeof(output_path));
    }
    ...
    status = fractus_legacy_bmp_save(output_path, &image, &palette);
    if (status != FRACTUS_STATUS_OK) {
        fprintf(stderr, "Error: No se pudo guardar el archivo BMP '%s'\n", output_path);
